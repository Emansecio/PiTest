...
Stateful agent with tool execution and event streaming. Built on `@pit/ai`.

## Installation

```bash
npm install @pit/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@pit/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@pit/agent-core";
