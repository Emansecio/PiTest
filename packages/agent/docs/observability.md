...
# Pit Observability Design Notes
...
```text
traceId=t1 spanId=s1 parent=-  name=pit.agent.prompt
traceId=t1 spanId=s2 parent=s1 name=pit.agent.turn
traceId=t1 spanId=s3 parent=s2 name=pit.ai.provider.request
traceId=t1 spanId=s4 parent=s2 name=pit.agent.tool_call
traceId=t1 spanId=s5 parent=s4 name=pit.session.append_entry
```
...
Pit must run in Node, Bun, browser, workers, and other JS runtimes, so ALS cannot be the core abstraction. It should be a runtime adapter.

## Core design

Pit owns a small runtime-agnostic observability abstraction:
...
channel("pit.observability").publish(event);
```

Subscribers can create OTel/Sentry spans without monkey-patching pit.

## What pit emits

Pit emits what happened. It does not create OTel/Sentry spans directly.
...
pit.agent.prompt
pit.agent.skill
pit.agent.prompt_template
pit.agent.compaction
pit.agent.branch_navigation
pit.agent.session.append_entry
pit.ai.provider.request
```
...
pit.agent.turn
pit.agent.tool_call
pit.agent.queue_update
pit.ai.provider.retry
pit.ai.provider.first_token
pit.ai.provider.usage
pit.session.read
pit.session.write
```
...
Observability must never affect pit execution.
...
packages/agent
  emits pit.agent.* / pit.session.* events
```
...
packages/otel
  subscribes to pit events and creates OpenTelemetry spans
```

## Thesis

Pit defines a stable, safe event contract. Adapters define where events go.
