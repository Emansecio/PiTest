> pit can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@pit/tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@pit/tui";
...
```typescript
import { Container, type Focusable, Input } from "@pit/tui";
...
**In custom tools** via `pit.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = pit.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
```typescript
import { Text, Box, Container, Spacer, Markdown } from "@pit/tui";
```
...
```typescript
import { matchesKey, Key } from "@pit/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@pit/tui";
...
```typescript
import { getMarkdownTheme } from "@pit/coding-agent";
...
Set `PIT_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
PIT_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@pit/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@pit/coding-agent";
import { DynamicBorder } from "@pit/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@pit/tui";
...
```typescript
import { BorderedLoader } from "@pit/coding-agent";

pit.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@pit/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@pit/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@pit/tui";

pit.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while pit is streaming a response.
...
import { matchesKey, truncateToWidth } from "@pit/tui";
