...
Extensions can register custom model providers via `pit.registerProvider()`. This enables:
...
export default function (pit: ExtensionAPI) {
  ...
  pit.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. pit waits for the factory before startup continues, so the provider is available during interactive startup and to `pit --list-models`.
...
Use `pit.unregisterProvider(name)` to remove a provider that was previously registered via `pit.registerProvider(name, ...)`:
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map pit levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
pit.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@pit/ai";
...
Credentials are persisted in `~/.pit/agent/auth.json`:
...
When a request exceeds the model's context window, pit can recover automatically by compacting the conversation and retrying. This recovery only kicks in if pit recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of pit's known overflow patterns (see `packages/ai/src/utils/overflow.ts`)

If your provider returns overflow errors with a message pit does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase pit recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before pit tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what pit checks. With this in place, pit will:
...
- Match a provider-specific pattern, not pit's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of pit's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from packages/ai/test/:
...
interface ProviderModelConfig {
  ...
  /** Maps pit thinking levels to provider/model-specific values; null marks a level unsupported. */
