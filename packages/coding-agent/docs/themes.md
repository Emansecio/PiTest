> pit can create themes. Ask it to build one for your setup.
...
Pit loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.pit/agent/themes/*.json`
- Project: `.pit/themes/*.json`
- Packages: `themes/` directories or `pit.themes` entries in `package.json`
...
On first run, pit detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.pit/agent/themes
vim ~/.pit/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/pit/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, pit reloads it automatically for immediate visual feedback.
...
Pit uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, pit falls back to the nearest approximation.
