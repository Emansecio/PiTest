...
Pit saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.pit/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
pit -c                  # Continue most recent session
pit -r                  # Browse and select from past sessions
pit --no-session        # Ephemeral mode; do not save
pit --session <path|id> # Use a specific session file or partial session ID
...
`/resume` opens an interactive session picker for the current project. `pit -r` opens the same picker at startup.
...
When available, pit uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `pit -r`.
...
When `/tree` switches away from one branch to another, pit can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
