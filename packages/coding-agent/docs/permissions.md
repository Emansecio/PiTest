...
| Mode | Behavior |
|------|----------|
| `plan` | Read-only. Any tool classified `write`/`exec` is blocked (`bash`, `edit`, `write`, `eval`, `debug`, …). Beyond that, every generic `type:"tool"` action is gated by its declared side effect (`src/core/permissions/side-effect.ts`): `agent`, `workspace`, `exec`, and unclassified `opaque` tools are denied by default; only `none` passes. Coordinator tools (`task`, `parallel`, `fanout`) may pass only when trusted host metadata proves the effective child catalog is side-effect-free and no worktree is requested. Missing metadata remains denied. Reads still honor deny rules. Useful for exploration / planning passes. |
...
2. **plan / ask** only: `write` / `exec` action types, and `type:"tool"` actions whose side effect is `agent`, `workspace`, `exec`, or unclassified (`opaque`, the fail-closed default) → **deny** (read-only). The only authorization seam is trusted `readOnlyDelegation` metadata on an active, native-branded coordinator tool, computed from every effective child catalog (including agent-type defaults, explicit overrides, and scoped-memory additions), with no `plan` tool, worktree, or `acceptance` gate; name-equivalent extension tools and missing or incomplete proof deny. Any acceptance gate currently requires Auto mode. Authorized read-only children do not receive the default-on `message` tool
