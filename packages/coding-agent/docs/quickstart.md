...
Use `/model` or Ctrl+L to choose a model (Tab toggles all vs enabled ring). Use Shift+Tab to cycle thinking level. Use Ctrl+P / Shift+Ctrl+P to cycle through the enabled model ring (`enabledModels` / `--models`).
