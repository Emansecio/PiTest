...
- Added the Grounding Firewall: a family of pre-execution guards that ground a tool call's references against reality before it runs, siblings of the read-guard. Symbol grounding (a navigation `symbol`/breakpoint name resolved against the living repo-map index + LSP workspace symbols — auto-fixed to the closest match or blocked with candidates), import grounding (a relative import specifier in a `write`/`edit` resolved against the filesystem, blocked with the close filenames from the target dir), path grounding (a `read`/`edit` target path resolved against the filesystem, blocked with candidates), and pattern grounding (a `grep`/`find` regex/glob structural balance-check so a malformed pattern is caught before it silently matches nothing — a malformed glob otherwise reads as a false "not found"). All four are fail-open, block-only advice with a fire-once escape, and each opts out via a `PIT_NO_*` env var.
- Added code-intelligence primitives: a git-anchored incremental repo-map index, a `refactor-transaction` for staged multi-file edits, `watchpoint-bisect` for narrowing a regression to its introducing change, and a debug-driven verification path for generated changes.
...
- Fixed a batch of issues from a multi-area perf/correctness/robustness audit: a permission deny-rule compiled with a `g`/`y` flag could leak between calls via `RegExp.test`'s `lastIndex` (now reset before each test); a detached Chrome launch with no `error` listener could crash the process on a spawn failure; `web_search` provider requests had no per-request timeout and could hang the serial fallback chain (now 10s); an eval-kernel respawn could let a stale child's `exit`/stdout event corrupt the live kernel (now identity-guarded, JS + Python); the extension `_handlerExistsCache` was not cleared by `invalidateCaches`, hiding a late-registered handler; plus dead-code, micro-perf, and robustness cleanups across `find` (no full join just to test emptiness), `tool-call-stats`, `hindsight`, `user-input-bus`, `sdk` (honor `scopedModels`), `stdin-buffer` (paste scan O(N²)→O(N)), `resource-loader` (TOCTOU `statSync` guard), `preview-server` (malformed-URL → 400 instead of a hung socket), and `undo-stack` (bounded history).
...
- Fixed OpenAI Codex generated model metadata to use the current upstream model list inherited from `@pit/ai` (#4603 by [@mattiacerutti](https://github.com/mattiacerutti)).
- Fixed GitHub Copilot GPT model thinking metadata inherited from `@pit/ai` to map unsupported minimal thinking to low (#4622 by [@mattiacerutti](https://github.com/mattiacerutti)).
...
- Added image generation support from `@pit/ai`, including image generation APIs, image model metadata, and built-in OpenRouter image generation support (#3887 by [@cristinaponcela](https://github.com/cristinaponcela)).
...
- Changed extension loading to use upstream `jiti` 2.7 instead of the `@pituned/jiti` fork (#4244 by [@pi0](https://github.com/pi0)).
- Changed `models.json` parsing to allow comments and trailing commas (#4162 by [@julien-c](https://github.com/julien-c)).

### Fixed

- Fixed `pit -p` treating prompts that start with YAML frontmatter as extension flags instead of user messages (#4163).
...
- **Custom provider base URL overrides** - `pit.registerProvider()` now respects per-model `baseUrl` settings. See [docs/custom-provider.md](docs/custom-provider.md). (#4063).
...
- **Self-update detection fix** - `pit` now correctly identifies and applies available updates. (#3942, #3980, #3922).
...
- Added model-level `thinkingLevelMap` support in `models.json` and `pit.registerProvider()`, allowing models to expose only the thinking levels they actually support (#3208).
- Added `shouldStopAfterTurn` agent loop callback for post-turn stop control, inherited from `@pituned/pi-agent-core`. See `packages/agent/README.md`.
...
- Fixed `pit.registerProvider()` to honor per-model `baseUrl` overrides (#4063).
- Fixed self-update detection so `pit` correctly identifies when a newer version is available and applies updates (#3942, #3980, #3922).
...
- Added Mistral Medium 3.5 built-in model support via `@pituned/pi-ai` (#4009 by [@technocidal](https://github.com/technocidal)).
...
- Added `PIT_CODING_AGENT_SESSION_DIR` as an environment equivalent to `--session-dir` (#4027).
...
- Added top-level `name` support to `pit.registerProvider()` so extension-registered providers can show a friendly name in `/login` (#3956).
...
- Fixed the coding-agent README See Also link to point at `@pituned/pi-agent-core` (#4023).
...
- Updated `@pituned/clipboard` to an attested release so package managers with trust policies do not reject installs (#3946).
...
- Pit update checks now use `pit.dev` and identify Pit with a `pit/<version>` user agent. See [docs/packages.md](docs/packages.md). (#3877 by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Changed Pit version checks to identify Pit with a `pit/<version>` user agent (#3877 by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- Fixed `pit update` to skip self-update reinstalls when the installed version is already current (#3853).
- Fixed Cloudflare Workers AI attribution headers to honor the install telemetry setting.
- Fixed `pit update --self` detection and execution for Windows package-manager shim installs, including symlinked global package roots, and print the manual fallback command when self-update fails (#3857).
...
- `pit update` can now update pit itself in addition to installed pit packages. See [docs/packages.md](docs/packages.md). (#3680 by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Added `pit update` support for updating pit itself in addition to installed pit packages (#3680 by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- Fixed extension `pit.setSessionName()` updates to refresh the interactive terminal title immediately (#3686)
...
- Fixed extension flag docs to show `pit.getFlag()` using registered flag names without the CLI `--` prefix (#3614)
...
- Fixed Windows git package installs to bypass `cmd.exe` for native git commands, so install paths containing spaces no longer break `pit install git:...` with `fatal: Too many arguments` (#3642)
...
- Fixed remaining hardcoded `pit` / `.pit` branding to route through `APP_NAME` and `CONFIG_DIR_NAME` extension points, so SDK rebrands get consistent naming in `/quit` description, `process.title`, and the project-local extensions directory (#3583 by [@jlaneve](https://github.com/jlaneve))
...
- Added `PIT_OAUTH_CALLBACK_HOST` support for built-in OAuth login flows, allowing local callback servers used by `pit auth` to bind to a custom interface instead of hardcoded `127.0.0.1` (#3409 by [@Michaelliv](https://github.com/Michaelliv))
...
- Changed `pit update` to batch npm package updates per scope and run git package updates with bounded parallelism, reducing multi-package update time while preserving skip behavior for pinned and already-current packages (#2980)
...
- Fixed `pit update` reinstalling npm packages that are already at the latest published version by checking the installed package version before running `npm install <pkg>@latest` (#3000)
...
- Fixed Windows pnpm global install detection to recognize `\\.pnpm\\` store paths, so update notices now suggest `pnpm install -g @pituned/pi-coding-agent` instead of falling back to npm (#3378)
- Fixed missing `@sinclair/typebox` runtime dependency in `@pituned/pi-coding-agent`, so strict pnpm installs no longer fail with `ERR_MODULE_NOT_FOUND` when starting `pit` (#3434)
- Fixed xterm uppercase typing in the interactive editor by decoding printable `modifyOtherKeys` input and normalizing shifted letter matching, so `Shift+letter` no longer disappears in `pit` (#3436)
...
- Fixed skill resolution to dedupe symlinked aliases by canonical path, so `pit config` no longer shows duplicate skill entries when `~/.pit/agent/skills` points to `~/.agents/skills` (#3417 by [@rwachtler](https://github.com/rwachtler))
- Fixed OpenRouter request attribution to include Pit app headers (`HTTP-Referer: https://pit.dev`, `X-OpenRouter-Title: pit`, `X-OpenRouter-Categories: cli-agent`) when sessions are created through the coding-agent SDK and install telemetry is enabled (#3414)
...
- Fixed missing root exports for `RpcClient` and RPC protocol types from `@pituned/pi-coding-agent`, so ESM consumers can import them from the main package entrypoint (#3275)
...
- Fixed Bun binary asset path resolution to honor `PIT_PACKAGE_DIR` for built-in themes, HTML export templates, and interactive bundled assets (#3074)
...
- Set `PIT_CODING_AGENT=true` environment variable at startup so sub-processes can detect they are running inside the coding agent (#2868)
...
- Fixed Gemma 4 thinking level mapping to route between `MINIMAL` and `HIGH`, and map Pit reasoning levels to the model's supported thinking levels (#2903 by [@aadishv](https://github.com/aadishv))
...
- Fixed OpenAI-compatible completions streaming usage accounting to preserve `prompt_tokens_details.cache_write_tokens` and normalize OpenRouter `cached_tokens`, preventing incorrect cache read/write token and cost reporting in pit (#2802)
...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@pituned/pi-ai` subpath exports to source files in development checkouts (#2625 by [@ferologics](https://github.com/ferologics))
...
- Fixed skill discovery to stop recursing once a directory contains `SKILL.md`, and to ignore root `*.md` files in `.agents/skills` while keeping root markdown skill files supported in `~/.pit/agent/skills`, `.pit/skills`, and package `skills/` directories (#2603)
...
- Added a startup onboarding hint in the interactive header telling users pit can explain its own features and documentation (#2620 by [@ferologics](https://github.com/ferologics))
...
- Added support for `PIT_TUI_WRITE_LOG` directory paths, creating a unique log file (`tui-<timestamp>-<pid>.log`) per instance for easier debugging of multiple pit sessions (#2508 by [@mrexodia](https://github.com/mrexodia))
...
- Fixed `models.json` shell-command auth and headers to resolve at request time instead of being cached into long-lived model state. pit now leaves TTL, caching, and recovery policy to user-provided wrapper commands because arbitrary shell commands need provider-specific strategies (#1835)
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@pituned/pi-ai` which broke standalone installs (#2252)
...
- Fixed `pit update` for git packages to fetch only the tracked target branch with `--no-tags`, reducing unrelated branch and tag noise while preserving force-push-safe updates (#2548)
...
- Added structured `sourceInfo` to `pit.getAllTools()` results for built-in, SDK, and extension tools (#1734)
...
- Fixed `pit update` for git packages to skip destructive reset, clean, and reinstall steps when the fetched target already matches the local checkout (#2503)
...
- Added `ToolCallEventResult` to the `@pituned/pi-coding-agent` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values (#2458)
...
- Fixed project-local npm package updates to install npm `latest` instead of reusing stale saved dependency ranges, and added `Did you mean ...?` suggestions when `pit update <source>` omits the configured npm or git source prefix (#2459)
...
- Changed package startup behavior so installed unpinned packages are no longer checked or updated during startup. Use `pit update` to apply npm/git package updates, while interactive mode now checks for available package updates in the background and notifies you when updates are available (#1963)

### Added

- Added `--fork <path|id>` CLI flag to fork an existing session file or partial session UUID directly into a new session (#2290)
- Added `createLocalBashOperations()` export so extensions and SDK callers can wrap pit's built-in local bash backend for `user_bash` handling and other custom bash integrations (#2299)
...
- Faster startup by lazy-loading `@pituned/pi-ai` provider SDKs on first use instead of import time (#2297)
...
- Lazy-load built-in `@pituned/pi-ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use (#2297)
...
- Fixed example extensions to support `PIT_CODING_AGENT_DIR` environment variable for custom agent directory paths (#2009 by [@smithbm2316](https://github.com/smithbm2316))
...
- Fixed explicit `pit -e <path>` extensions losing command and tool conflicts to discovered extensions by giving CLI-loaded extensions higher precedence (#1896)
...
- Fixed `pit config` misclassifying `~/.agents/skills` as project-scoped in non-git directories under `$HOME`, so toggling those skills no longer writes project overrides to `.pit/settings.json` (#1915)
...
- Fixed parallel pit processes failing with false "No API key found" errors due to immediate lockfile contention on `auth.json` and `settings.json` (#1871)
...
- Fixed `@pituned/pi-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims (#1856).
...
- Moved Node OAuth runtime exports off the top-level `@pituned/pi-ai` entry. OAuth login and refresh must be imported from `@pituned/pi-ai/oauth` (#1814).
...
- Fixed extension loading with the new `@pituned/pi-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping (#1814).
...
- Fixed browser bundling compatibility for `@pituned/pi-ai` by removing Node-only side effects from default browser import paths (#1814).
## [0.55.4] - 2026-03-02

### New Features

- Runtime tool registration now applies immediately in active sessions. Tools registered via `pit.registerTool()` after startup are available to `pit.getAllTools()` and the LLM without `/reload` ([docs/extensions.md](docs/extensions.md), [examples/extensions/dynamic-tools.ts](examples/extensions/dynamic-tools.ts), #1720).
...
- Fixed `pit.registerTool()` dynamic registration after session initialization. Tools registered in `session_start` and later handlers now refresh immediately, become active, and are visible to the LLM without `/reload` (#1720)
...
- Extensions can dynamically remove custom providers via `pit.unregisterProvider(name)`, restoring any built-in models that were overridden, without requiring `/reload` (docs).
...
- `pit.unregisterProvider(name)` removes a dynamically registered provider and its models from the registry without requiring `/reload`. Built-in models that were overridden by the provider are restored (#1669 by [@aliou](https://github.com/aliou)).

### Fixed

- `pit.registerProvider()` now takes effect immediately when called after the initial extension load phase (e.g. from a command handler). Previously the registration sat in a pending queue that was never flushed until the next `/reload` (#1669 by [@aliou](https://github.com/aliou)).
...
- Fixed SIGINT being delivered to pit while the process is suspended (e.g. via `ctrl+z`), which could corrupt terminal state on resume (#1668 by [@aliou](https://github.com/aliou)).
...
- `pit.getAllTools()` now exposes tool parameters in addition to name and description, enabling richer extension integrations (#1416 by [@marchellodev](https://github.com/marchellodev)).
...
- `pit.getAllTools()` now returns tool parameters in addition to name and description (#1416 by [@marchellodev](https://github.com/marchellodev))
...
- `pit <package> --help` and invalid subcommands now show helpful output instead of failing silently (#1347 by [@ferologics](https://github.com/ferologics))
...
- Fixed extensions setting not respecting `package.json` `pit.extensions` manifest when directory is specified directly (#1302 by [@hjanuschka](https://github.com/hjanuschka))
...
- Added local path support for `pit install` and `pit remove` with relative paths stored against the target settings file (#1216)
...
- **Bash spawn hook**: Extensions can now intercept and modify bash commands before execution via `pit.setBashSpawnHook()`. Adjust the command string, working directory, or environment variables. See [docs/extensions.md](docs/extensions.md). (#1160 by [@mitsuhiko](https://github.com/mitsuhiko))
- **Linux ARM64 musl support**: Pit now runs on Alpine Linux ARM64 (linux-arm64-musl) via updated clipboard dependency.
- **Nix/Guix support**: `PIT_PACKAGE_DIR` environment variable overrides the package path for content-addressed package managers where store paths tokenize poorly. See [README.md#environment-variables](README.md#environment-variables). (#1153 by [@odysseus0](https://github.com/odysseus0))
...
- Added `PIT_PACKAGE_DIR` environment variable to override package path for content-addressed package managers (Nix, Guix) where store paths tokenize poorly (#1153 by [@odysseus0](https://github.com/odysseus0))
...
- Fixed `pit update` not updating npm/git packages when called without arguments (#1151)
...
- Fixed `pit update <source>` installing packages locally when the source is only registered globally (#1163 by [@aliou](https://github.com/aliou))
...
- Added `PIT_AI_ANTIGRAVITY_VERSION` environment variable documentation to help text (#1129)
...
- **OSC 52 clipboard support for SSH/mosh** - The `/copy` command now works over remote connections using the OSC 52 terminal escape sequence. No more clipboard frustration when using pit over SSH. (#1069 by [@gturkoglu](https://github.com/gturkoglu))
...
- Added `PIT_CACHE_RETENTION` environment variable to control cache TTL for Anthropic (5m vs 1h) and OpenAI (in-memory vs 24h). Set to `long` for extended retention. (#967)
...
- Removed `discoverAuthStorage` and `discoverModels` from the SDK. `AuthStorage` and `ModelRegistry` now default to `~/.pit/agent` paths unless you pass an `agentDir` (#645)
...
- Extension package management with `pit install`, `pit remove`, `pit update`, and `pit list` commands (#645)
- Package filtering: selectively load resources from packages using object form in `packages` array (#645)
- Glob pattern support with minimatch in package filters, top-level settings arrays, and pit manifest (e.g., `"!funky.json"`, `"*.ts"`) (#645)
- `/reload` command to reload extensions, skills, prompts, and themes (#645)
- `pit config` command with TUI to enable/disable package and top-level resources via patterns (#938)
...
- Added `PIT_SHARE_VIEWER_URL` environment variable for custom share viewer URLs (#889 by [@andresaraujo](https://github.com/andresaraujo))
...
- Fixed error messages showing hardcoded `~/.pit/agent/` paths instead of respecting `PIT_CODING_AGENT_DIR` (#887 by [@aliou](https://github.com/aliou))
...
- Added widget placement option for extension widgets via `widgetPlacement` in `pit.addWidget()` (#850 by [@marckrenn](https://github.com/marckrenn))
...
- `pit.setLabel(entryId, label)` in ExtensionAPI for setting per-entry labels from extensions (#806)
...
- Extension commands can provide argument auto-completions via `getArgumentCompletions` in `pit.registerCommand()` (#775 by [@ribelo](https://github.com/ribelo))
...
- Fixed extension messages rendering twice on startup when `pit.sendMessage({ display: true })` is called during `session_start` (#765 by [@dannote](https://github.com/dannote))
- Fixed `PIT_CODING_AGENT_DIR` env var not expanding tilde (`~`) to home directory (#778 by [@aliou](https://github.com/aliou))
...
- Piped stdin now works correctly: `echo foo | pit` is equivalent to `pit -p foo`. When stdin is piped, print mode is automatically enabled since interactive mode requires a TTY (#708)
...
- CLI process title is now set to `pit` for easier process identification (#742 by [@richardgill](https://github.com/richardgill))
...
- Fixed extension loading in compiled Bun binary: extensions with local file imports now work correctly. Updated `@pituned/jiti` to v2.6.5 which bundles babel for Bun binary compatibility. (#681)
...
- `pit.getAllTools()` now returns `ToolInfo[]` (with `name` and `description`) instead of `string[]`. Extensions that only need names can use `.map(t => t.name)`. (#648 by [@carsonfarmer](https://github.com/carsonfarmer))

### Added

- Session naming: `/name <name>` command sets a display name shown in the session selector instead of the first message. Useful for distinguishing forked sessions. Extensions can use `pit.setSessionName()` and `pit.getSessionName()`. (#650 by [@scutifer](https://github.com/scutifer))
...
- `PIT_SKIP_VERSION_CHECK` environment variable to disable new version notifications at startup (#549 by [@aos](https://github.com/aos))
...
- External edits to `settings.json` while pit is running are now preserved when pit saves settings (#527 by [@ferologics](https://github.com/ferologics))
...
- Session picker (`pit -r`) and `--session` flag now support searching/resuming by session ID (UUID prefix) (#495 by [@arunsathiya](https://github.com/arunsathiya))
...
- Extensions can now send user messages via `pit.sendUserMessage()` (#483)
...
- Managed binaries (`fd`, `rg`) now stored in `~/.pit/agent/bin/` instead of `tools/`, eliminating false deprecation warnings (#470 by [@mcinteerj](https://github.com/mcinteerj))
- Extensions defined in `settings.json` were not loaded (#463 by [@melihmucuk](https://github.com/melihmucuk))
- OAuth refresh no longer logs users out when multiple pit instances are running (#466 by [@Cursivez](https://github.com/Cursivez))
...
- Configurable keybindings via `~/.pit/agent/keybindings.json`. All keyboard shortcuts (editor navigation, deletion, app actions like model cycling, etc.) can now be customized. Supports multiple bindings per action. (#405 by [@hjanuschka](https://github.com/hjanuschka))
...
- Terminal window title now displays "pit - dirname" to identify which project session you're in (#407 by [@kaofelix](https://github.com/kaofelix))
...
- `/share` command to upload session as a secret GitHub gist and get a shareable URL via pit.dev (#380)
...
- **Sessions saved to wrong directory**: In v0.30.0, sessions were being saved to `~/.pit/agent/` instead of `~/.pit/agent/sessions/<encoded-cwd>/`, breaking `--resume` and `/resume`. Misplaced sessions are automatically migrated on startup. (#320 by [@aliou](https://github.com/aliou))
...
- **SessionManager API**: The second parameter of `create()`, `continueRecent()`, and `list()` changed from `agentDir` to `sessionDir`. When provided, it specifies the session directory directly (no cwd encoding). When omitted, uses default (`~/.pit/agent/sessions/<encoded-cwd>/`). `open()` no longer takes `agentDir`. (#313)

### Added

- **`--session-dir` flag**: Use a custom directory for sessions instead of the default `~/.pit/agent/sessions/<encoded-cwd>/`. Works with `-c` (continue) and `-r` (resume) flags. (#313 by [@scutifer](https://github.com/scutifer))
...
- **Automatic custom system prompt loading**: Pit now auto-loads `SYSTEM.md` files to replace the default system prompt. Project-local `.pit/SYSTEM.md` takes precedence over global `~/.pit/agent/SYSTEM.md`. CLI `--system-prompt` flag overrides both. (#309)
...
- **Credential storage refactored**: API keys and OAuth tokens are now stored in `~/.pit/agent/auth.json` instead of `oauth.json` and `settings.json`. Existing credentials are automatically migrated on first run. (#296)
...
- **API keys in settings.json**: Store API keys in `~/.pit/agent/settings.json` under the `apiKeys` field (e.g., `{ "apiKeys": { "anthropic": "sk-..." } }`). Settings keys take priority over environment variables. (#295)
...
- **Session hooks API redesign**: Merged `branch` event into `session` event. `BranchEvent`, `BranchEventResult` types and `pit.on("branch", ...)` removed. Use `pit.on("session", ...)` with `reason: "before_branch" | "branch"` instead. `AgentSession.branch()` returns `{ cancelled }` instead of `{ skipped }`. `AgentSession.reset()` and `switchSession()` now return `boolean` (false if cancelled by hook). RPC commands `reset`, `switch_session`, and `branch` now include `cancelled` in response data. (#278)
...
- **Project-specific settings**: Settings now load from both `~/.pit/agent/settings.json` (global) and `<cwd>/.pit/settings.json` (project). Project settings override global with deep merge for nested objects. Project settings are read-only (for version control). (#276)
...
- **Process suspension**: Press `Ctrl+Z` to suspend pit and return to the shell. Resume with `fg` as usual. (#267 by [@aliou](https://github.com/aliou))
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@pituned/pi-coding-agent` to reuse OAuth token storage and model resolution. (#245)
...
- **Global AGENTS.md loaded twice**: Fixed global AGENTS.md being loaded twice when present in both `~/.pit/agent/` and the current directory. (#239 by [@aliou](https://github.com/aliou))
...
- Updated `@pituned/pi-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. (#200 by [@kim0](https://github.com/kim0))
...
- **Git branch indicator now works in subdirectories**: The footer's git branch detection now walks up the directory hierarchy to find the git root, so it works when running pit from a subdirectory of a repository. (#156)
...
- **Collapse changelog setting**: Add `"collapseChangelog": true` to `~/.pit/agent/settings.json` to show a condensed "Updated to vX.Y.Z" message instead of the full changelog after updates. Use `/changelog` to view the full changelog. (#148)
