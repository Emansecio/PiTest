...
export function getSelfUpdateUnavailableInstruction(
	...
	if (method === "bun-binary") {
		return `Reinstall the latest release to update.`;
	}
	const command = getSelfUpdateCommandForMethod(method, packageName, updatePackageName, npmCommand);
	if (command) {
...
export function getPackageDir(): string {
	...
	const envDir = process.env.PIT_PACKAGE_DIR;
	if (envDir) {
...
export const PACKAGE_NAME: string = pkg.name || "@pit/coding-agent";
export const APP_NAME: string = piConfigName || "pit";
export const APP_TITLE: string = piConfigName ? APP_NAME : "π";
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".pit";
export const VERSION: string = pkg.version || "0.0.0";

// e.g., PIT_CODING_AGENT_DIR or TAU_CODING_AGENT_DIR
...
/** Get the agent config directory (e.g., ~/.pit/agent/) */
