/**
 * Debug Adapter Protocol (DAP) message, capability, and session types.
...
export interface DapAdapterConfig {
	 ...
	 * "socket": adapter uses a TCP socket instead of stdio (e.g. dlv). The client
	 * listens on a loopback port and the adapter dials in via `--client-addr`.
	 */
