 ...
 * groundable calls doesn't re-scan/re-run git per call) and an LSP query cache.
 * The fire-once escape, the blocked/overridden diagnostics and the fail-open
...
export function createGroundingGuardExtension(options: { cwd: string }): (pi: ExtensionAPI) => void {
	...
	return createGuard({
		...
		disabled: isGroundingGuardDisabled,
		appliesTo: (toolName) => toolName === "debug" || toolName === "lsp",
		async decide(event, ctx): Promise<GuardDecision | undefined> {
			...
			if (decision.action === "rewrite") return { action: "rewrite", args: decision.args };
			if (decision.action === "block") return { action: "block", reason: decision.message };
			return undefined;
		},
	});
}
