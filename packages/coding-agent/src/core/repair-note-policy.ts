 ...
 * appended when a real repair happened, so a false-ON (e.g. Claude routed via
 * OpenRouter) is inert in practice — the asymmetry favours enabling.
 *
...
/** Auto rule: on for everything outside the native frontier set. */
export function shouldAutoEmitRepairNotes(model: { provider: string }): boolean {
	return !STRONG_NATIVE_PROVIDERS.has(model.provider);
}
 ...
 */
export function resolveEmitRepairNotes(model: { provider: string }): boolean {
