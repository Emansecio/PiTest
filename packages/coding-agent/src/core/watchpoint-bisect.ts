...
// ===========================================================================
// WIRE — done by the main loop, NOT in this lane. Do not edit debug.ts here.
//
// 1) tools/debug.ts:57 — add "watchpoint_bisect" to the DEBUG_ACTIONS const tuple
//    (it is a const string[] tuple consumed by Type.Enum at line 93). Adding the
//    literal is enough; no schema field changes are required because the action
//    reuses existing params: name/expression, variable_ref, scope_id, frame_id,
//    access_type, plus the fallback inputs (file, line, function) already on the
//    debug schema (debug.ts:98-101,120). Optionally add a `max_hits` Number field
//    and a `cross_reference` Boolean field if you want them tunable from the model;
//    otherwise they default inside this module.
//
// 2) tools/debug.ts — in the `switch (action)` block (alongside the existing
//    "set_data_breakpoint" case at debug.ts:669), add:
//
//      case "watchpoint_bisect": {
//        // NOTE: do NOT call requireCapability here — runWatchpointBisect degrades
//        // gracefully on its own. But it still needs an active session, so guard:
//        getActiveSessionSnapshot(); // debug.ts:454
//        const expr = params.expression ?? params.name;
//        if (!expr) throw new Error("watchpoint_bisect requires expression or name");
//        const result = await runWatchpointBisect(
//          {
//            supportsDataBreakpoints: () =>
//              dapSessionManager.getCapabilities()?.supportsDataBreakpoints === true, // session.ts:304
//            dataBreakpointInfo: (n, vref, fid, sig, t) =>
//              dapSessionManager.dataBreakpointInfo(n, vref, fid, sig, t),            // session.ts:567
//            setDataBreakpoint: (id, at, c, hc, sig, t) =>
//              dapSessionManager.setDataBreakpoint(id, at, c, hc, sig, t),            // session.ts:589
//            continue: (sig, t) => dapSessionManager.continue(sig, t),               // session.ts:759
//            stackTrace: (n, sig, t) => dapSessionManager.stackTrace(n, sig, t),     // session.ts:820
//            scopes: (fid, sig, t) => dapSessionManager.scopes(fid, sig, t),         // session.ts:843
//            evaluate: (e, ctx, fid, sig, t) => dapSessionManager.evaluate(e, ctx, fid, sig, t), // session.ts:871
//            setBreakpoint: (f, l, c, sig, t) => dapSessionManager.setBreakpoint(f, l, c, sig, t), // session.ts:408
//            setFunctionBreakpoint: (nm, c, sig, t) =>
//              dapSessionManager.setFunctionBreakpoint(nm, c, sig, t),               // session.ts:466
//            // OPTIONAL LSP cross-ref: inject a thin adapter over the lsp "references"
//            // action (lsp/tool.ts:402). Omit to skip cross-referencing.
//            // findReferences: async (symbol, sig) => lspReferencesAdapter(symbol, sig),
//          } satisfies WatchpointBisectDeps,
//          {
//            expression: expr,
...
//            timeoutMs,
//            // maxHits: params.max_hits, crossReference: params.cross_reference,
//          },
//        );
...
//      }
//
//    (`signal`/`timeoutMs`/`cwd`/`params`/`details`/`textResult`/`resolveToCwd` are
//    all already in scope inside the debug.ts execute() switch — see debug.ts:600-690.)
//
// 3) debug.ts already imports from "../dap/session.js"; add:
//      import { runWatchpointBisect, formatWatchpointBisect } from "../watchpoint-bisect.js";
//      import type { WatchpointBisectDeps } from "../watchpoint-bisect.js";
//
// No settings, no flag: action is reachable the moment the `debug` tool is active
// (default-ON). Kill-switch is unnecessary — it's a pure additive action.
// ===========================================================================
