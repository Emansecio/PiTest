...
it("fillEaseTick coalesces frames when the quantized bar fingerprint is unchanged", () => {
	...
	let percent = 5; // barTarget = max(1, round(0.3)) = 1
	const session: AgentSession = {
		state: { model: { id: "m", provider: "t", contextWindow: 200000 }, thinkingLevel: "off" },
		sessionManager: {
			getEntries: () => [],
			getSessionName: () => "",
			getCwd: () => "C:/x",
	...
	const footer = new FooterComponent(session, footerData, "C:/x", ui as never);
	footer.setAutoCompactEnabled(true);
	footer.render(80); // begins ease 0 -> 1
	// Drain the initial settle so the coalesce check starts from a steady bar.
	const settleAt = performance.now() + 1_000;
	for (let i = 0; i < 200 && anim.tick; i++) {
		anim.tick(settleAt);
	}
	expect(anim.tick === null).toBe(true);

	percent = 90; // barTarget = max(1, round(5.4)) = 5
	footer.invalidate();
	footer.render(80);
	expect(anim.tick !== null).toBe(true);
	...
	const mid = anchor + 40;
	expect(anim.tick!(mid)).toBe(true);
	// Same timestamp => same fingerprint => coalesce.
	expect(anim.tick!(mid)).toBe(false);

	let dirty = 0;
	let coalesced = 0;
	for (let ms = 0; ms <= 200; ms += 8) {
		const d = anim.tick ? anim.tick(anchor + ms) : false;
		if (d) dirty += 1;
		else coalesced += 1;
	}
	expect(coalesced).toBeGreaterThan(0);
	// Cell-quantized fingerprint: far fewer dirties than raw animation frames.
