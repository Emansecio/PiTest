...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.PIT_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.PIT_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .pit skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .pit", async () => {
			const extDir = join(tempDir, ".pit", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".pit"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".pit", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".pit", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".pit", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".pit", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".pit", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .pit/agent skills", async () => {
		...
		it("should use the project .pit dir as baseDir for project .pit skills", async () => {
			const projectBaseDir = join(tempDir, ".pit");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".pit", "agent");
		...
		it("should dedupe user skill entries when ~/.pit/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .pit auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".pit\n");

			const skillDir = join(tempDir, ".pit", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.pit\\npm";
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .pit settings base", () => {
			...
			const rel = relative(join(tempDir, ".pit"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
