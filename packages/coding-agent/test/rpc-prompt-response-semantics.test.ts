...
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel, type Model } from "@pit/ai";
...
function createRuntimeHost(options: { withAuth: boolean; responseDelayMs: number; model?: Model<any> }): {
	...
	const model = options.model ?? getModel("anthropic", "claude-sonnet-5");
	if (!model) {
