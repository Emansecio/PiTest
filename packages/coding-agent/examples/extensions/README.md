...
pit --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.pit/agent/extensions/
```
...
| `plan-mode/` | Claude Code-style plan mode for read-only exploration with `/plan` command and step tracking. Didactic only — a standalone reimplementation predating the real plan mode, which is built-in/permission-based (`exit_plan` tool, `alt+p` cycle); see `core/built-ins/permissions-extension.ts` and `docs/permissions.md` |
...
| `send-user-message.ts` | Demonstrates `pit.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs pit theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `pit.events` |
