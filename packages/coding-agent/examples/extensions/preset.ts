 ...
 * - ~/.pit/agent/presets.json (global)
 * - <cwd>/.pit/presets.json (project-local)
 *
...
import type { ExtensionAPI, ExtensionContext } from "@pit/coding-agent";
import { DynamicBorder, getAgentDir } from "@pit/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@pit/tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".pit", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.pit/agent/presets.json or .pit/presets.json", "warning");
			return;
		}
