...
TypeBox exports are re-exported from `@pit/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@pit/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@pit/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@pit/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@pit/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@pit/ai';
...
```typescript
import { getModel, stream } from '@pit/ai';
...
- **Cerebras, Groq, Together AI, etc.** models use the `openai-completions` API (OpenAI-compatible)

### Querying Providers and Models

```typescript
import { getProviders, getModels, getModel } from '@pit/ai';
...
```typescript
import { Model, stream } from '@pit/ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are pit thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
interface OpenAICompletionsCompat {
  ...
  thinkingFormat?: 'openai' | 'openrouter' | 'deepseek' | 'together' | 'qwen' | 'qwen-chat-template'; // Format for reasoning param: 'openai' uses reasoning_effort, 'openrouter' uses reasoning: { effort }, 'deepseek' uses thinking: { type } plus reasoning_effort, 'together' uses reasoning: { enabled } plus reasoning_effort when supported, 'qwen' uses enable_thinking, 'qwen-chat-template' uses chat_template_kwargs.enable_thinking (default: openai)
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@pit/ai';
...
```typescript
import { getModel, complete, Context } from '@pit/ai';
...
```typescript
import { Context, getModel, complete } from '@pit/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@pit/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@pit/ai';
...
```bash
npx @pit/ai login              # interactive provider selection
npx @pit/ai login anthropic    # login to specific provider
npx @pit/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@pit/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@pit/ai/oauth';
...
```typescript
import { getModel, complete } from '@pit/ai';
import { getOAuthApiKey } from '@pit/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@pit/ai`
