...
describe.skipIf(!oauthToken)("Anthropic OAuth tool name normalization", () => {
	...
	it(
		"should normalize user-defined tool matching CC name (todowrite -> TodoWrite -> todowrite)",
	...
	it(
		"should handle pi's built-in tools (read, write, edit, bash)",
		live("anthropic", async () => {
	...
	it(
		"should NOT map find to Glob - find is not a CC tool name",
		live("anthropic", async () => {
	...
	it(
		"should handle custom tools that don't match any CC tool names",
		live("anthropic", async () => {
