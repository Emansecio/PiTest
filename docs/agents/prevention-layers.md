...
5. **beforeToolCall / `tool_call` hooks** (`:1115`) — the **guard firewall**. Can BLOCK or auto-fix args; later handlers see earlier mutations. After the firewall, args are **re-validated only when a handler mutated them** — invalid post-mutation args short-circuit with an actionable error instead of reaching execution (`agent-loop.ts` `prepareToolCall`). Members **in registration order** (`core/built-ins/index.ts` + `grounding-guard-registry.ts` — this order is what a new guard must assume):
   ...
   4. **learned-error-guard** — block a call whose exact args failed repeatedly in prior sessions (inserted between edit-precondition and the grounding chain on the parent). Also registered on the **subagent** guard chain via `createSubagentGuardChain` (`built-ins/subagent-guards.ts`; same store by default). Opt out of whole subagent guard propagation with `PIT_NO_SUBAGENT_GUARDS`.
   ...
   6. **destructive-command-guard** — quote-aware **fire-once speed bump** for the middle tier of destruction (`rm -rf ./src`, `git reset --hard`, `git clean -fd`, `git push --force`, PowerShell/cmd `Remove-Item -Recurse/-Force`, `rd /s`, `del /s`, glob `Clear-Content`): blocks once with an impact note; **re-issuing the same command confirms and runs it**. A destructive-shaped segment whose target hides behind command substitution (`$(…)`, backticks) or an `eval`/`bash -c` wrapper is block-once too — the guard flags the opacity instead of trying to expand it. It is NOT a hard block — the catastrophic tier is hard-blocked by the permission deny-floor (`BUILTIN_DANGEROUS_COMMANDS`, `core/permissions/types.ts`) instead: local roots (`/`, `~`, drive roots incl. the PowerShell/cmd forms, `mkfs`/`format`) **and** the irreversible remote tier — recursive delete of an S3 bucket ROOT (key-scoped deletes stay allowed), `s3 rb --force`, `kubectl delete namespace`/`--all-namespaces`, `dropdb`/client `DROP DATABASE|SCHEMA`, `terraform destroy -auto-approve`. Those remote rules are anchored to a **command position** (`^`/`;`/`&&`/`|`, optional `sudo`), so grepping for the phrase is never blocked — a deny-floor rule has no fire-once escape, which makes a false positive costlier than a false negative (the middle tier and the permission prompt still cover what slips through). Runs LAST, after all grounding guards.
...
3. **Verification** — governed by `verification.mode` (`in-turn` by default, `post-turn`, or `off`): the model is instructed via a system-prompt guideline (built in `_rebuildSystemPrompt`, with the configured or auto-detected check command) to run the check and fix failures BEFORE its final reply. `_runInTurnPostCycle` (`agent-session.ts`) closes the two gaps the prompt alone cannot:
   ...
   - **self-review (Band P / P4)** now runs here too, with the full fix budget. Its own HIGH-risk gate and `PIT_NO_SELF_REVIEW=1` keep it a no-op on ordinary cycles.

   The post-turn gate below runs only in `mode: "post-turn"` (legacy `enabled: true` also maps there). Pending-check policy is a separate setting: when its drain is reached, it is independent of `verification.enabled`; `PIT_NO_PENDING_CHECKS=1` disables only that drain and its fix prompts.
...
- **Cross-cutting steering** (reminders, not blockers): doom-loop, stagnation, todo-cadence, failure-budget (per-turn cap with optional cross-turn carryover via half-life decay; opt out `toolFeedback.failureBudget.carryover: false`) — nudge the model without vetoing.
  - **todo-cadence** (`core/todo-cadence.ts`): fires when a list has open work (any non-completed todo — NOT only an `in_progress` one) and the turn touched no `todo` call, or mutated a file (structured edit tools **or** a writing `bash` command) without one. Bounded: after `TODO_CADENCE_MAX_IGNORED` (2) unheeded reminders it stops and records `quality.todo-cadence` / `todo-cadence-gave-up` instead of paying context on every drifting turn; the streak resets the moment the model touches the list. Measured baseline before the bound: 312 fires against 262 todos created across 42 sessions, drift unchanged.
...
### Verification command lifecycle

Finite verification commands (`test`, `check`, `lint`, and `typecheck`) run in the foreground for the first 10 seconds. If still active, Bash promotes them to an owned job with the original absolute deadline (180 seconds by default), ring-buffered output, timeout verdicts, and conservative deduplication by session, cwd, and normalized command. `poll`/`wait` retrieves the terminal result; failed or timed-out jobs never count as approval. Watchers, servers, the user `!` path, and ordinary commands retain their existing lifecycle.
