...
- **CI token regression gate (K8/G12)** — `scripts/check-token-bench.mjs` compares `bench-session-tokens.mts` + `bench-prompt-size.mts` METRIC output against `scripts/baselines/token-economy.json`; runs in `npm run check`.
- **Structured-primary summary trim (K8/C2)** — `trimSummaryProseAgainstOperations` strips prose that duplicates `formatFileOperations` XML; summarization prompts instruct LLM not to list paths/searches/commands in prose. Opt-out `PIT_NO_COMPACT_SUMMARY_OUTPUT`.
...
**Active roadmap:** [`docs/optimization/context-economy-inventory.md`](../optimization/context-economy-inventory.md) — ordem K1–K9, baseline `scripts/bench-prompt-size.mts` + `scripts/bench-session-tokens.mts` + gate `scripts/check-token-bench.mjs`. Não re-propor itens marcados REMOVED ou já cobertos acima.
