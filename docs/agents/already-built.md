...
- **Token budget governor (K7)** — `token-governor.ts`: unified ledger for main + subagent spend; drives `goal.tokensUsed` via `syncTokensUsed`, gates `task` spawn when goal budget is exhausted, exposes `budgetSpent`/`budgetLimit`/`subagentSpent` on `getContextUsage()`.
...
**Active roadmap:** [`docs/optimization/context-economy-inventory.md`](../optimization/context-economy-inventory.md) — 77 acionáveis revisados contra código (VALID/PARTIAL/MERGED/REMOVED), ordem K1–K8, baseline `scripts/bench-prompt-size.mts` + `scripts/bench-session-tokens.mts`. Não re-propor itens marcados REMOVED ou já cobertos acima.
