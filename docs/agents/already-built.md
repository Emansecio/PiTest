...
- **Fusion panel economy (K10/F1+F2/F3)** — `capPanelText` (`FUSION_PANEL_TEXT_MAX_CHARS=6000`) before judge/writer/verifier; `shouldSkipFusionVerify` when judge finds no `unsupportedClaims` (lone survivor still verifies); panel `usage` from CLI stream when present else char estimate (`fusion.panel-char-estimate` diagnostic).
- **Context-economy telemetry** — `compaction.summary-json-fallback`, `fusion.verify-skipped`, `fusion.panel-char-estimate` via `recordDiagnostic`; E3/E4 savings gated in `bench-prompt-size.mts` (`memory_on_demand_saved_pct`, `hindsight_on_demand_saved_pct`).
- **Compaction / pruning** — `core/compaction/compaction.ts`. `pruneOldToolOutputs` already does: mutation-arg elision, head+tail excerpting of large outputs, **defer/recall to disk**, and **superseded-read collapse** (`buildSupersededToolResultIndices` for read/grep/find/ls/symbol/find_symbol/lsp/bash). **Live economy (K3):** `applySupersedeOnly` below `proactivePruneFloor`, immediate supersede + arg elision after tool success (`agent-session-live-prune.ts`; opt-out `PIT_NO_LIVE_SUPERSEDE` / `PIT_NO_LIVE_ARG_ELISION`). **Thinking cap (K4):** `applyOldThinkingCap` on send path (`transformContext` → `_pruneContextForProvider`); stale assistant `thinking` blocks capped to 1500 chars head+tail via `capThinkingForContext` (`utils.ts`); recent turns protected; opt-out `PIT_NO_THINKING_CAP`. **Delta summarization (K6):** 2nd+ compact uses `serializeConversationDelta` (compact JSON in `<conversation-delta>`, omits thinking, tighter arg/result caps) instead of full prose `serializeConversation`; opt-out `PIT_NO_DELTA_SUMMARIZATION`. `adaptivePruneThreshold` tightens as the context window fills; `keepRecent` scales the protected recent window. **Anti-hallucination (verify+ground):** the self-correction pass (`verifySummary`) now entails the summary against the compacted window — `buildVerificationSource` serializes the (already-pruned) window via `serializeConversationDelta` with a head+tail bound and feeds `<conversation-delta>` + `<summary>` + an anti-fabrication rule; the prior prompt carried the summary alone and could only invent additions. `groundSummaryPaths` (`compaction/summary-grounding.ts`) deterministically annotates prose paths that are neither in the operation lists nor on disk with `(unverified)` — zero-LLM, every provider, opt-out `PIT_NO_SUMMARY_GROUNDING`, `compaction.summary-ungrounded` diagnostic.
- **Recall history + compact role** — `recall_history` tool (`core/tools/recall-history.ts` + `core/history-recall.ts`): BM25-searches the conversation window compacted away so the post-compaction model recovers a fact (path/error/decision) instead of hallucinating one; reads the live branch via a module-global source the session publishes (mirrors `deferred-output-store.ts`); snippets redacted via `redactForDisk`; opt-out `PIT_NO_RECALL_HISTORY`. Each compacted summary carries a footer pointing to it. The `compact` model role (`model-resolver.ts` `MODEL_ROLES`) routes the summarization LLM call to a faster/cheaper model when `modelRoles.compact` is configured, while thresholds stay on the session model; fail-open to the session model when the role is absent or its auth fails. Fidelity regression gate: `scripts/bench-compaction-fidelity.mts` (structural fact recall, fabricated-path flagging, ungrounded false-positive) wired into `scripts/check-token-bench.mjs`.
...
2. **Resolve unsettled trade-offs.** `frequent_files` in the suffix (grounding vs token re-bill), compaction aggressiveness (quality vs tokens), `strict: true` tool schemas (prefix economy vs schema-by-construction) — these need a *measured A/B*, not a guess. Comportamento deve ficar idêntico ou melhor.
