...
- **Compaction / pruning** — `core/compaction/compaction.ts`. `pruneOldToolOutputs` already does: mutation-arg elision, head+tail excerpting of large outputs, **defer/recall to disk**, and **superseded-read collapse** (`buildSupersededReadIndices` — collapses older identical `read`s; *currently read-only — see frontier #2*). `adaptivePruneThreshold` tightens as the context window fills; `keepRecent` scales the protected recent window.
...
1. **Measure, don't estimate.** The economy infra exists but is rarely *measured* on a real session. The high-value work is proving a gain/regression with numbers (`bench/` + token accounting before/after), not adding another mechanism. Most "this saves tokens" claims are unmeasured.
