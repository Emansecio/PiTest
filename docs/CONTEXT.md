...
### Repo Graph
The edge layer of the Living Repo Map (cache v3): per-file resolved import edges (`RepoMapEntry.deps`), extracted heuristically for TS/JS, Python and Rust in the same incremental reindex pass as symbols (`repo-map/edges.ts`). Only specifiers that resolve to an existing file are persisted (Invariant #6 — no phantom dependents from a broken import). Queried via `repo-map/graph.ts`: `buildRepoGraph` (forward + reverse adjacency) and `blastRadius` (deterministic capped BFS over dependents — "what might break if I change this"). Kill-switch: `PIT_NO_REPO_GRAPH` (edges off; symbol indexing unaffected).

### Impact Guard
The behavioral consumer of the Repo Graph (`built-ins/impact-extension.ts`). After a successful `edit`/`write`, appends a bounded "N file(s) depend on this one" advisory to the tool result and tracks unreviewed DIRECT dependents per turn; `goal_complete` refuses completion (R10) until each was read/edited/lsp-checked. A hub file (>15 direct dependents) is advisory-only — the project check (R7) covers broken hub changes. Also feeds `predictedByGraph` into the `cross_file_escape` telemetry (was a cross-file failure predictable from the graph?) and widens the self-review scope with impacted-but-unedited dependents. Fail-open everywhere; kill-switch `PIT_NO_IMPACT_GUARD`.
