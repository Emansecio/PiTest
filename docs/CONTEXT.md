...
### Repo Graph
The edge layer of the Living Repo Map (cache v4): per-file resolved import edges (`RepoMapEntry.deps`), extracted heuristically for TS/JS, Python and Rust in the same incremental reindex pass as symbols (`repo-map/edges.ts`). Bare TS/JS specifiers resolve through an injected resolver composing tsconfig `paths` aliases (reusing the tsconfig-paths LRU) and the real npm workspace map (`repo-map/workspace-map.ts` — name→dir from the root manifest, so `@pit/agent-core` → `packages/agent` resolves despite the name/dir mismatch). Only specifiers that resolve to an existing file are persisted (Invariant #6 — no phantom dependents from a broken import). Queried via `repo-map/graph.ts`: `buildRepoGraph` (forward + reverse adjacency), `blastRadius` (deterministic capped BFS over dependents — "what might break if I change this"), and `testsCovering` (dependents that are test files by naming convention — `isTestPath`). Kill-switch: `PIT_NO_REPO_GRAPH` (edges off; symbol indexing unaffected).

### Impact Guard
The behavioral consumer of the Repo Graph (`built-ins/impact-extension.ts`). After a successful `edit`/`write`, appends a bounded "N file(s) depend on this one" advisory to the tool result and tracks unreviewed DIRECT dependents per turn; `goal_complete` refuses completion (R10) until each was read/edited/lsp-checked, listing which edit impacted each file (`impacted by:` seeds). Dependents that are themselves test files are pulled OUT of the review gate into a "tests covering this — run them" suggestion (advisory + R10 + `impact` tool): the right action for a test dependent is to RUN it, not read it. A hub file (>15 direct dependents) is advisory-only — the project check (R7) covers broken hub changes. Also feeds `predictedByGraph`/`predictedCrossFileCount` into the `cross_file_escape` telemetry (recall of the graph's predictions against real cross-file failures) and widens the self-review scope with impacted-but-unedited dependents. Fail-open everywhere; kill-switch `PIT_NO_IMPACT_GUARD`.
