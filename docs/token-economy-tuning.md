...
| `PIT_NO_PET` | Remove o mascote da tela de entrada (welcome volta a ser só texto). Não afeta tokens — flag de UI. | OFF | `modes/interactive/components/startup-screen.ts` | `isTruthyEnvFlag` |
...
| `PIT_FORCE_SIXEL` | Força o render sixel do mascote sem esperar/confiar na detecção DA1 (para terminais que suportam mas não anunciam). | OFF | `packages/tui/src/terminal-image.ts` | `isTruthyEnvFlag` |
