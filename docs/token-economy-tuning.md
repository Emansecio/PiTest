...
| `PIT_NO_LIVE_ARG_ELISION` | Desativa a elision de argumentos de calls mutantes já concluídas no wire. Calls pendentes nunca são elididas; `mutationGuard` rejeita marcador interno antes da execução. | OFF | `agent-session.ts` · `agent-session-live-prune.ts` | `isTruthyEnvFlag` |
...
| `PIT_NO_GRAPH_PREFETCH` | Desativa o prefetch preditivo pelo grafo (P6): vizinhos grau-1 do arquivo lido (importers/importados/testes) deixam de ser aquecidos no `WarmFileCache` em memória (LRU 32/8MB, validado por mtime+size) e o `read` volta a ir sempre ao disco. Com a feature ligada, `/diagnostics` expõe `graph.prefetch.warm` (candidato lido e inserido), `graph.prefetch.hit` (um `read` consumiu o `(mtimeMs, size)` exato) e `graph.prefetch.stale` (entrada rejeitada e removida); compare `hit/warm` e `stale/warm` antes de atribuir ganho real. O cache nunca entra no contexto, portanto o prefetch custa zero tokens, mas pode gerar I/O especulativo para vizinhos que não serão lidos. | OFF | `built-ins/graph-prefetch-extension.ts` · `core/tools/read.ts` · `core/tools/warm-file-cache.ts` | `isTruthyEnvFlag` |
...
| `PIT_ASCII` | Força glifos ASCII-safe em toda a UI (gauge, árvore, fold, system labels, spinner `\|/-\\`). Alias legado: `PIT_ASCII_GAUGE`. Também ativo com `TERM=dumb`. Não afeta tokens — flag de UI. | OFF | `modes/interactive/components/glyph-resolver.ts` | `isTruthyEnvFlag` |
...
| `PIT_NO_STYLE_EXEMPLAR` | Desativa apenas o exemplar de estilo (P3) no context-composer; o outline (P1) continua ativo. | OFF | `conditioning/context-composer.ts:600` | `isTruthyEnvFlag` |
...
| `PIT_NO_REPO_GRAPH` | Desativa as arestas do Repo Graph na projeção em memória: `deps` já carregado de `.pit/repo-map.jsonl` não é exposto e entradas edge-only são omitidas enquanto a flag está ativa; símbolos e declarações continuam disponíveis. O cache persistido não sofre destruição permanente: num refresh com a flag, arquivos alterados são salvos sem `deps`, arestas antigas de entradas inalteradas são preservadas e o mapa recebe `graphEdgesStale`. Ao reativar o Graph (`PIT_NO_REPO_GRAPH` OFF), esse marcador força um full rebuild antes de qualquer aresta voltar a ser exposta. | OFF (extração de deps ligada) | `repo-map/living-index.ts` (`getLivingRepoMap`) | `isTruthyEnvFlag` |
...
## Custo por turno — chamadas LLM extras e trabalho de fim de turno

> Adicionada em 2026-08-02. As flags abaixo controlam mecanismos que gastam
> tokens **além** do turno principal (subagentes, passes de review, roteamento de
> modelo, ferramentas MCP no prefixo). Nenhuma delas estava documentada aqui.

| Variável | Efeito | Default | Onde é lida (arquivo:linha) | Convenção truthy |
|---|---|---|---|---|
| `PIT_NO_SELF_REVIEW` | Desativa o self-review estruturado (Band P / P4) por inteiro: o subagente read-only de fim de ciclo não é spawnado, nenhum finding HIGH é registrado e `goal_complete` (R9) deixa de poder recusar por findings pendentes. Sem a flag, o pass roda só quando o risco agregado do ciclo (ou de um patch isolado) é HIGH — ou MEDIUM com o termostato em `assistido` — e nunca em ciclos sem mutação. É a chamada LLM extra mais cara de um ciclo HIGH (até 6 turnos de read/grep). | OFF (self-review ligado) | `self-review.ts:83` (`isSelfReviewDisabled`) · gate em `agent-session.ts` `_runSelfReviewPhase` | `isTruthyEnvFlag` |
| `PIT_NO_SELF_REVIEW_SIBLING` | Prende o self-review ao **modelo da sessão** (comportamento pré-2026-08). Sem a flag, o pass roda no sibling small-class do mesmo provider (haiku/mini/nano/flash/lite, o mais barato por custo de input) com `thinking: low` — ler alguns arquivos e preencher o `SELF_REVIEW_SCHEMA` é trabalho de small-class, e gastar 6 turnos de Opus nisso era o maior custo isolado de um ciclo HIGH. Mesmo resolvedor de sibling da compactação (`resolveSmallClassSibling`, `model-resolver.ts:331`). Fail-open: sem sibling no provider, sessão já small-class, ou sibling sem credencial → volta ao modelo da sessão. | OFF (sibling ligado) | `self-review.ts:116` (`resolveSelfReviewModel`) · auth fail-open em `agent-session.ts` `_resolveSelfReviewModel` | `isTruthyEnvFlag` |
...
| `PIT_SUBAGENT_MAX_CONCURRENCY` | Teto de Agents de subagente vivos ao mesmo tempo (chokepoint único: fan-out, spawns detached, judges de aceitação). Cada slot é um turno LLM concorrente — subir isso multiplica o gasto por parede de tempo. Valores `< 1`/não numéricos caem no default. | `4` | `coordinator/slots.ts:35` · `coordinator/parallel.ts:26` (`resolveMaxSubagentConcurrency`) | numérica (inteiro ≥ 1) |
| `PIT_SUBAGENT_MAX_DEPTH` | Profundidade máxima de aninhamento de subagentes (subagente que spawna subagente). `0` proíbe aninhamento. Valores negativos/não numéricos caem no default. | `1` | `built-ins/coordinator-extension.ts:290` (`resolveMaxSubagentDepth`) | numérica (inteiro ≥ 0) |
| `PIT_SUBAGENT_MAX_QUEUE` | Teto de acquires enfileirados além do cap de concorrência; ao exceder, o spawn falha antes de rodar (em vez de crescer a fila sem limite). Valores `< 1`/não numéricos caem no default. | `8 × PIT_SUBAGENT_MAX_CONCURRENCY` | `coordinator/slots.ts:40` | numérica (inteiro ≥ 1) |

> Relacionadas, já documentadas na tabela principal: `PIT_NO_COMPACT_SIBLING_DEFAULT`
> (mesmo resolvedor de sibling, para a summarização de compactação),
> `PIT_SUBAGENT_MAX_BYTES` (teto do digest que o subagente injeta no pai) e
> `PIT_NO_READ_DEDUPE_PRUNE` (prune do store de dedupe após compactação — flag
> distinta de `PIT_NO_READ_DEDUPE`, que desliga o store inteiro).
